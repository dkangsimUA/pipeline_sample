...
# SET COUNTRY CODE FOR CORRECT COUNTRY, MEX
country_code <- "MEX"
...
files <- files[str_detect(files, glue("^{country_code}_"))] # select Mexico results
...
json_data <- read_json(here("3_qualtrics_survey_upload", 
                            "qualtricsSurveyConfig.json"))

## Each time we run this, the script updates the json file with current info
## Instructions for the API tokens are found in 
#'Qualtrics Import and Assign
...
json_data %>% 
  ...
  write_file(here("3_qualtrics_survey_upload", 
                  "qualtricsSurveyConfig.json"))
...
source_python(here("3_qualtrics_survey_upload", 
                   "1_subscripts", 
                   "1_upload_surveys.py"))
...
source_python(here("3_qualtrics_survey_upload", 
                   "1_subscripts", "3_collaborate_surveys.py"))
beep(sound = 5) # play sound when done running
...
assign_files <- list.files(here("3_qualtrics_survey_upload", 
                                "3_assign_results")) # find list of all survey results
assign_files <- assign_files[str_detect(assign_files, 
                                        glue("^result-survey-assignment_{country_code}_"))] # select correct results
...
if (country_code == "MEX") {
  email_mex <- render_email(here('4_drupal_upload','email_country_managers.Rmd') ,
                            render_options = list(params = list(data = "MEX", 
                                                                name = "Kristal")))
  email_mex
  smtp_send(
    email_mex, # the email that you just rendered
